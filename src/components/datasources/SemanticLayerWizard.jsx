...
const STEPS = ['Select Sources', 'Define Entities', 'Map Fields', 'Review & Create'];
...
Keep entity names as business concepts (e.g. "Customer", "Order", "Product", "Asset").`,
    ...
    if (result?.suggestedEntities) {
      setEntities(result.suggestedEntities.map(e => ({ name: e.name, description: e.description })));
    }
  ...
  const handleCreate = async () => {
    ...
    setIsCreating(false);
    toast.success(`Semantic layer "${layerName}" created successfully`);
    handleClose();
  };
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 0 && (
              <motion.div key="step0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea value={layerDescription} onChange={e => setLayerDescription(e.target.value)} placeholder="Describe the business purpose of this semantic layer…" className="mt-1 h-16 resize-none" />
                </div>
                <div>
            ...
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                  ...
                  {entities.map((entity, i) => (
                    <div key={i} className="bg-card border border-border/60 rounded-lg p-3 space-y-2">
                      <div className="flex items-center gap-2">
                        <Input
                          value={entity.name}
                          onChange={e => setEntities(prev => prev.map((en, idx) => idx === i ? { ...en, name: e.target.value } : en))}
                          placeholder="Entity name (e.g., Customer)"
                          className="h-8 text-xs flex-1"
                        />
                        {entities.length > 1 && (
                          <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => setEntities(prev => prev.filter((_, idx) => idx !== i))}>
                            ×
                          </Button>
                        )}
                      </div>
                      <Input
                        ...
                        onChange={e => setEntities(prev => prev.map((en, idx) => idx === i ? { ...en, description: e.target.value } : en))}
                ...
                <Button size="sm" variant="outline" className="gap-1.5 h-8 text-xs" onClick={() => setEntities(prev => [...prev, { name: '', description: '' }])}>
                  + Add Entity
                </Button>
              </motion.div>
            )}
            ...
            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                {entities.filter(e => e.name.trim()).map(entity => {
                    ...
                    <div key={entity.name} className="bg-card border border-border/60 rounded-lg p-4">
                      ...
                      {mappings.length > 0 ? (
                        ...
                        <div className="text-center py-4 text-muted-foreground">
                          <p className="text-xs">No AI field suggestions — mappings will be auto-inferred on first sync</p>
                        </div>
                      )}
                    </div>
                  );
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="bg-card rounded-lg p-3">
                      ...
                      <p className="text-sm font-bold">{entities.filter(e => e.name.trim()).length}</p>
                      <p className="text-[10px] text-muted-foreground mt-1">{entities.filter(e => e.name.trim()).map(e => e.name).join(', ')}</p>
                    </div>
                  </div>
                </div>

                <div className="space-y-2">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Entities to Create</p>
                  {entities.filter(e => e.name.trim()).map(entity => (
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border/40 mt-4">
          ...
          <div className="flex items-center gap-2">
            {step === 0 && (
              <Button
                size="sm"
                disabled={!canProceedStep0 || isGenerating}
                onClick={handleGenerateSuggestions}
                className="gap-2"
              >
                {isGenerating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                {isGenerating ? 'Analyzing Sources…' : 'Analyse & Continue'}
              </Button>
            )}
            {step === 1 && (
            ...
            {step === 3 && (
              <Button size="sm" disabled={isCreating} onClick={handleCreate} className="gap-2 bg-emerald-600 hover:bg-emerald-700 text-white">
                {isCreating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Layers className="w-4 h-4" />}
                {isCreating ? 'Creating…' : 'Create Semantic Layer'}
              </Button>
            )}
          </div>
        </div>
